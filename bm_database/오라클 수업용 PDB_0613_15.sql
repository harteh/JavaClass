...
SELECT NVL(STU_HEIGHT, 0) FROM STUDENT;
-- 신장열의 값이 널인 학생의 경우 미기록으로 검색
SELECT NVL(TO_CHAR(STU_HEIGHT), '미기록') FROM STUDENT;
SELECT NVL(TO_CHAR(STU_HEIGHT), ' ') FROM STUDENT;

SELECT STU_NO, STU_NAME, (STU_HEIGHT + STU_WEIGHT) AS 합, STU_HEIGHT,STU_WEIGHT FROM STUDENT;
SELECT STU_NO, STU_NAME, NVL(STU_HEIGHT, 0) + STU_WEIGHT AS 합
FROM STUDENT;

-- 신장이 NULL인 경우 '입력요망'으로 바꿔 학번, 이름을 검색
SELECT STU_NO, STU_NAME , NVL(TO_CHAR(STU_HEIGHT), '입력요망') AS 키 FROM STUDENT;
-- 학생의 성별을 NULLIF로 비교하여, 남학생인 경우 성별과 '남자'로 검색하라
...
SELECT STU_DEPT AS 학과,COUNT(*) FROM STUDENT 
GROUP BY STU_DEPT ORDER BY COUNT(*) DESC;
-- 73. 학년별 학생들의 인원수를 검색하라.
SELECT STU_GRADE AS 학년, COUNT(*) FROM STUDENT GROUP BY STU_GRADE;
-- 74. 학과별 학년별 학생들의 평균 체중을 검색하라.
SELECT STU_DEPT, STU_GRADE, AVG(STU_WEIGHT) AS 평균체중 FROM STUDENT
GROUP BY STU_DEPT, STU_GRADE ORDER BY STU_DEPT;
-- 75. 학과별 학년별 학생들의 학번의 MAX와 MIN값을 검색하라.
SELECT STU_DEPT AS 학과, STU_GRADE AS 학년,
MAX(STU_NO), MIN(STU_NO)
FROM STUDENT
GROUP BY STU_DEPT, STU_GRADE ORDER BY STU_DEPT;
